...
import telran.java38.student.dao.exceptions.StudentNotFoundException;
...
public class StudentServiceImpl implements StudentService {
	...
	public StudentDto findStudentById(Integer id) {
		...
		return converToStudentDto(student);
	}
	...
	private StudentDto converToStudentDto(Student student) {
	...
	public StudentBaseDto updateStudent(Integer id, StudentUpdateDto studentUpdateDto) {
		...
		studentRepository.save(student);
		return converToStudentBaseDto(student);
	}
	...
	public List<StudentDto> findStudentsByGroup(String group) {
		return studentRepository.findByGroup(group).map(s -> converToStudentDto(s)).collect(Collectors.toList());
	}

	@Override
	public List<StudentDto> findStudentsByParameters(String group, String name) {
		return studentRepository.findByGroupAndName(group, name).map(s -> converToStudentDto(s))
	...
	public List<StudentDto> findStudentsByExamScore(String exam, int minScore) {
		return studentRepository.findByExamMinScore(exam, minScore).map(s -> converToStudentDto(s))
				.collect(Collectors.toList());
	}
